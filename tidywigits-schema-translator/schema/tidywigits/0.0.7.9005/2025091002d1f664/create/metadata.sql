CREATE EXTERNAL TABLE IF NOT EXISTS tidywigits.metadata (
  `input_id` STRING,
  `output_id` STRING,
  `input_dirs` ARRAY<STRING>,
  `output_dir` STRING,
  `pkg_versions` ARRAY<STRUCT<name: STRING, version: STRING>>,
  `files` ARRAY<STRUCT<tbl: STRING, prefix: STRING, fout: STRING, fin: STRING>>
)
PARTITIONED BY (
    batch_date string
)
