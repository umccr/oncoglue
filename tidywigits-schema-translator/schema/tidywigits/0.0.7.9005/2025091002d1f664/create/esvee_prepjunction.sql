CREATE EXTERNAL TABLE IF NOT EXISTS tidywigits.esvee_prepjunction (
  `input_id` STRING,
  `input_prefix` STRING,
  `output_id` STRING,
  `chrom` STRING,
  `position` DOUBLE,
  `orientation` DOUBLE,
  `junc_frags` DOUBLE,
  `exact_support_frags` DOUBLE,
  `other_support_frags` DOUBLE,
  `low_map_qual_frags` DOUBLE,
  `max_qual` DOUBLE,
  `extra_info` DOUBLE,
  `indel` STRING,
  `hotspot` STRING,
  `initial_read_id` STRING
)
PARTITIONED BY (
    batch_date string
)
STORED AS PARQUET
LOCATION 's3://derived-secondary-data-123456789123-ap-southeast-2-an/tidywigits/v1'
TBLPROPERTIES (
    'projection.enabled'='true',
    'projection.batch_date.type'='date',
    'projection.batch_date.range'='2026-01-01,NOW',
    'projection.batch_date.format'='yyyy-MM-dd',
    'storage.location.template'='s3://derived-secondary-data-123456789123-ap-southeast-2-an/tidywigits/v1/batch_date=${batch_date}/'
);
